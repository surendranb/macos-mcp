...
server.setRequestHandler(types_js_1.ListToolsRequestSchema, async () => {
    (0, telemetry_js_1.captureEvent)('list_tools').catch(console.error);
    return { tools: TOOLS };
});
// Register call tool handler
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    (0, telemetry_js_1.captureEvent)('tool_executed', { tool: request.params.name }).catch(console.error);
    ...
    try {
        ...
        `;
                ...
                if (description)
                    fullScript += `set description of newEvent to "${escape(description)}"\n`;
                if (location)
                    fullScript += `set location of newEvent to "${escape(location)}"\n`;
                fullScript += `end tell`;
        ...
        `;
          ...
          `;
                ...
                const props = [`name:"${escape(title)}"`];
                if (notes)
                    props.push(`body:"${escape(notes)}"`);
                script += `
        ...
        `);
            ...
            case 'send_notification': {
                const { title, subtitle, body } = args;
        ...
        `);
            ...
            case 'take_screenshot': {
                const { path: savePath, interactive = false, type = 'screen' } = args;
        ...
        `);
            ...
            case 'resize_window': {
                const { app, x, y, width, height, windowIndex = 1 } = args;
                await runAppleScript(`
          tell application "System Events"
            tell process "${escape(app)}"
              try
...
async function run() {
    ...
    (0, telemetry_js_1.captureEvent)('mcp_started').catch(console.error);
